...
class SaprodRep extends CI_Controller {
	...
	{	
		...
		$this->template->load_Sadmin_view('prodRep');
	
	}
	public function fetch_product(){  
           $this->load->model("ProdRepmodel");  
           $fetch_data = $this->ProdRepmodel->make_datatables();
           ...
           {  
                ...
                if ( $row->status == 'IN' ) {
                    $sub_array[] = '<span style="color: green"><b>'.$row->status.'</b></span>';
                }else{
                    $sub_array[] = '<span style="color: red"><b>'.$row->status.'</b></span>';
                }  
           ...
           $output = array(  
                ...
                "recordsTotal" => $this->ProdRepmodel->get_all_data(),  
                "recordsFiltered" => $this->ProdRepmodel->get_filtered_data(),  
                "data" => $data  
           );  
	...
	{	
		$this->load->model("Superadminmodel");
		$data = $this->Superadminmodel->fetch_product_log_interval($_POST["startDate"],$_POST["endDate"]);
		$file = fopen("C:/Users/PC/Desktop/Product_Report/".date('d-m-Y-H-m')."_productReport.csv", 'w+');
		$header = array('Product Name','Total Qty','Time','Action');
		fputcsv($file, $header);
